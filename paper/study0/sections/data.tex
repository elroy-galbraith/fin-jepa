...
The resulting universe contains approximately 8{,}000 unique companies. Each company-year observation is identified by its CIK (Central Index Key), ticker symbol, fiscal year, fiscal period end date, and 10-K filing date. While the universe spans fiscal years 2012--2024, all modeling in this paper uses observations through fiscal year 2023: the 365-day label horizon (\Cref{sec:labels}) requires up to a year of post-period data, so 2024 fiscal years cannot be fully labeled within our 2024-12-31 data boundary. After merging with the XBRL feature matrix, the modeling dataset comprises 33{,}737 company-year observations (\Cref{sec:splits}).
...
\begin{align*}
  ...
  X_3 &= \text{operating income} / \text{total assets}, &
  X_4 &= \text{equity} / \text{total liabilities}, \\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Quantile transformation.} We apply scikit-learn's \texttt{Quantile\allowbreak Transformer} with \texttt{output\_distribution='normal'} and up to 1{,}000 quantiles (minimum~2), which maps the empirical rank distribution of each feature to a standard normal distribution. This is the default normalization method; z-score standardization is available as an alternative.
\end{enumerate}
...
For each company-year observation, we align market data to the 10-K filing date and compute forward returns at four horizons: 21, 63, 126, and 252 trading days. Returns are market-adjusted by subtracting the corresponding S\&P~500 (\^{}GSPC) return over the same window, and sector-adjusted using Fama--French 12-industry sector ETF returns. We compute 63-day average daily volume preceding the filing date as an additional feature. Companies with insufficient post-filing price data (indicating delisting) are flagged. These forward returns enter the dataset only through the \texttt{stock\_decline} label (\Cref{sec:labels}); they are never used as model features, so the market-derived outcome introduces no look-ahead into the predictors.
...
The first two outcomes (\texttt{stock\_decline} and \texttt{earnings\_restate}) are derived entirely from publicly available EDGAR data. The remaining two require external datasets: the Dechow et al.\ AAER database for SEC enforcement actions and the UCLA-LoPucki Bankruptcy Research Database for bankruptcy filings. When external label files are not present, the corresponding label columns are all-NaN and the outcome is excluded from evaluation. Delisted companies with insufficient post-filing price data are treated as stock decline events.

Two caveats apply to the externally sourced labels. First, the UCLA--LoPucki Bankruptcy Research Database was frozen in December 2022 and covers only large public filers, so \texttt{bankruptcy} labels under-represent small-firm failures and the most recent filings. Second, SEC accounting-enforcement actions are typically issued two to three years after the violation period \citep{karpoff2017proxies}, so our 365-day labelling horizon captures only the fastest cases and right-censors recent test years. This censoring, together with the small positive count, makes \texttt{sec\_enforcement} the least reliable of the four outcomes; we weight it accordingly throughout and revisit a longer-horizon definition in future work.

On the test split (14{,}160 company-years), positive/negative counts are: \texttt{stock\_decline} 8{,}334/5{,}821 (5 company-years lack sufficient post-filing prices and are unlabeled), \texttt{earnings\_restate} 2{,}952/11{,}208, \texttt{sec\_enforcement} 96/14{,}064, and \texttt{bankruptcy} 113/14{,}047. Only the two genuinely rare outcomes (\texttt{sec\_enforcement} at 0.68\% and \texttt{bankruptcy} at 0.80\% positive, fewer than 120 positives each) behave as low-base-rate problems; \texttt{earnings\_restate} (20.8\% positive) and \texttt{stock\_decline} (58.9\%) do not. The small positive counts on the two rare outcomes drive the wide confidence intervals reported in the results.
