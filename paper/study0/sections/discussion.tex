...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Robustness of the headline advantage.} Walk-forward validation across seven expanding windows (each with its preprocessing fit on its own training split) shows the FT-Transformer's edge on \texttt{earnings\_restate}---the highest-signal of the rare outcomes---holds in every fold, including the two COVID-era windows, rather than being an artifact of the single 2020--2023 test split. The bankruptcy advantage is also largely robust (6/7 folds), narrowing only in the 2020--2021 window.
\end{enumerate}
...
Several limitations should be noted. Although the FT-Transformer clears the AUROC gate against fully tuned baselines (3/4 wins), the margins are uneven and the \texttt{sec\_enforcement} win warrants particular caution. That result is unstable: AUROC varies substantially across seeds (multi-seed std $= 0.044$, ranging 0.486--0.593), a traditional-ratio logistic regression outperforms every other model on it, and the FT-Transformer wins only 4 of 7 walk-forward folds---all reflecting the extremely low base rate (AUPRC $< 0.02$). The \texttt{bankruptcy} outcome is steadier (multi-seed std $= 0.011$, 6/7 walk-forward folds) but its bootstrap CI against default XGBoost still includes zero. The most robust advantage is on \texttt{earnings\_restate}, which holds across all seven walk-forward folds with low seed variance. Finally, the test period (2020--2023) spans the COVID-19 market disruption, which provides a valuable out-of-distribution stress test but also means the test distribution may differ systematically from the training distribution.
