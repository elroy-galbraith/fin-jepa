...
\paragraph{Feature Tokenizer.} Each scalar input feature $x_i$ is projected to a $d$-dimensional token embedding via a per-feature affine transformation: $\mathbf{t}_i = \mathbf{W}_i x_i + \mathbf{b}_i$, where $\mathbf{W}_i \in \mathbb{R}^d$ and $\mathbf{b}_i \in \mathbb{R}^d$ are learnable parameters. Feature weights are initialized with Kaiming uniform initialization ($a = \sqrt{5}$). A learnable \textsc{[CLS]} token $\mathbf{t}_{\mathrm{cls}} \in \mathbb{R}^d$ is prepended to the token sequence. Categorical features---here the single \texttt{sector\_idx} feature (Fama--French 12-industry, cardinality~12)---are tokenized with a learned per-category embedding table rather than the affine map, and appended to the sequence; with $n$ continuous features the encoder therefore processes $n + 2$ tokens (continuous tokens, one categorical token, and \textsc{[CLS]}).
...
\paragraph{Hyperparameter selection.} We conduct a grid search over 36 configurations: learning rate $\in \{10^{-3}, 3{\times}10^{-4}, 10^{-4}, 3{\times}10^{-5}\}$, number of layers $L \in \{2, 3, 4\}$, and token dimension $d \in \{128, 192, 256\}$, with the remaining hyperparameters held at their defaults. The best configuration is selected by mean test AUROC across all evaluable outcomes. \emph{Because this selection touches the held-out test set, the resulting sweep (\Cref{fig:sweep_heatmap}) and the FT-Transformer column of \Cref{tab:auroc} are optimistic and are reported only as an exploratory diagnostic of hyperparameter sensitivity; they are not the basis for any claim in this paper.} All other training settings (optimizer, early stopping, batch size) are held constant across the sweep. For the like-for-like tuned comparison (\Cref{sec:results_tuned})---which underpins the gate, multi-seed, and walk-forward analyses---we tune the FT-Transformer with Optuna (30 trials, TPE sampler) over the same learning-rate, layer, and token-dimension ranges ($\eta \in [3{\times}10^{-5},\,10^{-3}]$ log-uniform, $L \in \{2,3,4\}$, $d \in \{128,192,256\}$), selecting on 3-fold expanding-window temporal cross-validation---the identical protocol used for the baselines. This procedure selects $d = 128$, $L = 3$, $\eta \approx 10^{-3}$, and these are the hyperparameters used for every headline result.
...
\paragraph{Histogram Gradient-Boosted Trees (GBT).} Scikit-learn's \texttt{HistGradientBoosting\-Class\-i\-fier} with 500 iterations, learning rate 0.05, max depth~6, minimum 20 samples per leaf, balanced class weighting. This model natively handles NaN values, making it suitable for evaluation on raw XBRL features with minimal preprocessing.

All baseline hyperparameters use default (untuned) values for the initial benchmark (\Cref{tab:auroc}). For the like-for-like tuned comparison (\Cref{sec:results_tuned}), each baseline receives Optuna temporal cross-validation tuning (30 trials, 3-fold expanding-window CV) over the following search spaces: XGBoost---\texttt{n\_estimators} $\in [100, 1000]$, \texttt{learning\_rate} $\in [0.01, 0.3]$ (log), \texttt{max\_depth} $\in [3, 10]$, \texttt{subsample} and \texttt{colsample\_bytree} $\in [0.5, 1.0]$; GBT---\texttt{max\_iter} $\in [100, 1000]$, \texttt{learning\_rate} $\in [0.01, 0.3]$ (log), \texttt{max\_depth} $\in [3, 10]$, \texttt{min\_samples\_leaf} $\in [10, 50]$; logistic regression---$C \in [10^{-3}, 10^{2}]$ (log).

\paragraph{Class imbalance.} Logistic regression and GBT use balanced class weighting, and the FT-Transformer applies a positive-class weight of $n_{\text{neg}}/n_{\text{pos}}$ in its \texttt{BCEWithLogitsLoss}. XGBoost is the exception: it uses \texttt{scale\_pos\_weight}${}=1$ (unweighted), and this is not part of its tuning search space. This asymmetry should be borne in mind when comparing calibration (ECE, Brier) on the low-base-rate outcomes, where XGBoost's unweighted objective contributes to its near-zero predicted probabilities.
...
\paragraph{Pretraining protocol.} SSL pretraining uses the sweep architecture ($d = 192$; \Cref{sec:ft_transformer}); because the SSL study is a scratch-versus-pretrained comparison at a fixed architecture, this choice does not bias the SSL effect, but it does differ from the cross-validation-selected architecture ($d = 128$) used for the gate. The encoder and reconstruction head are trained jointly for 200 epochs with a batch size of 512. We use AdamW optimization with learning rate $10^{-4}$ and weight decay $10^{-5}$. The learning rate follows a linear warmup over 10 epochs followed by cosine annealing to zero. Pretraining uses all available training-split data regardless of label availability.
...
The final benchmark evaluates five models head-to-head on each distress outcome: (1)~logistic regression on traditional financial ratios (LR-trad), (2)~logistic regression on the full feature set (LR-full), (3)~XGBoost on the full feature set, (4)~histogram gradient-boosted trees on raw XBRL features (GBT-raw), and (5)~FT-Transformer trained from scratch with tuned hyperparameters. Self-supervised pretraining is evaluated as a separate ablation on the FT-Transformer (\Cref{sec:results_ssl}). All models are evaluated on the identical held-out test split using the metrics defined in \Cref{sec:metrics}.
