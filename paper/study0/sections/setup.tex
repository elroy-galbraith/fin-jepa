...
As a robustness check, we also define a walk-forward evaluation scheme using expanding training windows. Starting with a train cutoff of 2014-12-31, each successive fold advances by one year, with a 1-year validation window and a 2-year test window, up to a final test boundary of 2023-12-31. This yields seven overlapping train/test splits (test windows from 2016--2017 through 2022--2023) that assess temporal stability of model performance across different market regimes, including the COVID-19 boundary. In the results reported here, feature normalization statistics are fit on data up to 2017-12-31 and shared across folds; for the two earliest folds (train cutoffs 2014 and 2015) this exposes the scaler to feature marginals from periods that overlap their test windows---a feature-level dependence shared by both models that we revisit in \Cref{sec:limitations}. Walk-forward results are reported in \Cref{sec:results_walkforward}.
...
To quantify initialization variance, we report multi-seed results for the FT-Transformer across three random seeds (42, 123, 456). The primary results use seed 42; the multi-seed analysis (\Cref{sec:results_multiseed}) assesses the stability of conclusions across initializations.
